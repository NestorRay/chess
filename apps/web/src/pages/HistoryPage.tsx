...
import type { GameHistory } from "@xiangqi/contracts";
import { api } from "../api";
...
export function HistoryPage() {
  ...
  return (
    <section className="history-page">
      ...
      {!isLoading && games.length === 0 && <div className="empty-state compact"><CalendarClock size={32} /><h2>还没有对局记录</h2><Link className="primary-button" to="/">开始第一局</Link></div>}
      ...
      {hasNextPage && (
        <div className="history-more">
          <button className="secondary-button" disabled={isFetchingNextPage} onClick={() => void fetchNextPage()}>
